...
public class GPGSBinder
{
    ...
    //https://forum.unity.com/threads/google-play-playgamesclientconfiguration-problems.1278653/
    //void Init() // 11.01패치이후 creat instance와 initialization, activation of playgameplatform을 할 필요없어짐
    //{
    ...
    public void Login()
    {
        ...
        {
            if(callback == SignInStatus.Success) 
            {
                PlayGamesPlatform.Instance.RequestServerSideAccess(true, code => { SetAuthenticationInformation(true); });
            }
            else
            {
    ...
    void SetAuthenticationInformation(bool isAuthenticated)
    {
        Debug.Log("isAuthenticated : " +isAuthenticated);
    }
    // 11.01부터 sign-in, sign-out 의 ingame button 불필요
    //public void Logout()
    //{
        ...
        SavedGame.OpenWithAutomaticConflictResolution(fileName, DataSource.ReadCacheOrNetwork,
            ConflictResolutionStrategy.UseLastKnownGood, (status, game) =>
            {
                if (status == SavedGameRequestStatus.Success)
                {
                    ...
                    update.WithUpdatedDescription(DateTime.Now.ToString("g"));
                    byte[] bytes = System.Text.Encoding.UTF8.GetBytes(saveData);

                    SavedGame.CommitUpdate(game, update.Build(), bytes, (status2, game2) =>
                    {
    ...
    public void LoadCloud(string fileName, Action<bool, string, ISavedGameMetadata> onCloudLoaded = null)
    {
        SavedGame.OpenWithAutomaticConflictResolution(fileName, DataSource.ReadCacheOrNetwork,
            ConflictResolutionStrategy.UseLastKnownGood, (status, game) =>
            {
                if (status == SavedGameRequestStatus.Success)
                {
                    SavedGame.ReadBinaryData(game, (status2, loadedData) =>
                    {
                        if (status2 == SavedGameRequestStatus.Success)
                        {
                            ...
                            onCloudLoaded?.Invoke(true, data, game);
                        }
                        else
                            onCloudLoaded?.Invoke(false, null, null);
                    });
                }
            });
    }
